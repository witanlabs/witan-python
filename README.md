...
pip install git+ssh://git@github.com/witanlabs/witan-python.git
```
...
to add `WITAN_API_KEY="My API Key"` to your `.env` file
...
```sh
# install from the production repo
pip install 'witan[aiohttp] @ git+ssh://git@github.com/witanlabs/witan-python.git'
```
...
When the library is unable to connect to the API (for example, due to network connection problems or a timeout), a subclass of `witan.APIConnectionError` is raised.

When the API returns a non-success status code (that is, 4xx or 5xx
response), a subclass of `witan.APIStatusError` is raised, containing `status_code` and `response` properties.

All errors inherit from `witan.APIError`.
...
except witan.RateLimitError as e:
    print("A 429 status code was received; we should back off a bit.")
except witan.APIStatusError as e:
